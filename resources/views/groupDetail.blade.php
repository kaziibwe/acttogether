    ...
    <section>
        ...
        <div id="group_ui_component"
            ...
            <div class="display">
                ...
                {{-- <a href="{{ route('adminEditProfile',$admin->id) }}"><button type="button"
                        class="text-white bg-red-400 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Edit Profile
                    </button> </a>

                    <a href="{{ route('changePassword',$admin->id) }}"><button type="button"
                        class="text-white bg-red-400 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Change Password
                    </button> </a>
                                ...
                                <div class="grid grid-cols-3">
                                    <div class="text-center my-2">
                                        <img class="h-30 w-40  mx-auto"
                                            src="{{ $group->image ? asset('storage/' . $group->image) : asset('/avatar.jpeg') }}"
                                            alt="">

                                        <a href="#" class="text-main-color">{{ $group->group_name }}</a>
                                    </div>
                                    {{-- <div class="text-center my-2">
                                ...
                                <div class="text-gray-700">
                                    <div class="grid md:grid-cols-2 text-sm">
                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Group name</div>
                                            <div class="px-4 py-2"> {{ $group->group_name }} </div>
                                        ...
                                        <div class="grid grid-cols-2">
                                            ...
                                            <div class="px-4 py-2"> {{ $group->group_location }}</div>
                                        </div>
                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Bank Account</div>
                                            <div class="px-4 py-2">{{ $group->group_bank_account }}</div>
                                        </div>
                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Contact No.</div>
                                            <div class="px-4 py-2"> {{ $group->group_phone }}</div>
                                        </div>
                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Number of Members</div>
                                            @php
                                                $members = \App\Models\Member::where('group_id', $group->id); // Fetch all members in the group
                                                $memberCount = $members->count();
                                            @endphp
                                            <div class="px-4 py-2">{{  $memberCount }} members</div>
                                        </div>

                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Total Group saving</div>

                                            @php
                                                $totalAmount = \App\Models\Member::where('group_id', $group->id)->sum(
                                                    'total_amount',
                                                ); // Sum the money of all members in the group
                                            @endphp


                                            <div class="px-4 py-2">Sh{{ $totalAmount }}</div>
                                        </div>
                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Treasurer</div>
                                            <div class="px-4 py-2">
                                                @if ($group->treasurer_id)
                                                    ...
                                                    @if ($treasurer)
                                                        <p>{{ $treasurer->savers_surname }}
                                                            {{ $treasurer->savers_given_name }}</p>
                                                    @endif
                                                @else
                                                    <code>
                                                        <a href="{{ route('admin.editGroup', ['id' => $group->id]) }}"
                                                            class="color-blue">Assign treasurer</a>
                                                    </code>
                                        ...
                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Secretary</div>
                                            <div class="px-4 py-2">
                                                @if ($group->secretary_id)
                                                    ...
                                                    @if ($secretary)
                                                        <p>{{ $secretary->savers_surname }}
                                                            {{ $secretary->savers_given_name }}</p>
                                                    @endif
                                                @else
                                                    <code>
                                                        <a href="{{ route('admin.editGroup', ['id' => $group->id]) }}"
                                                            class="color-blue">Assign secretary</a>
                                                    </code>
                                        ...
                                        <div class="grid grid-cols-2">
                                            <div class="px-4 py-2 font-semibold">Chairman</div>
                                            <div class="px-4 py-2">
                                                ...
                                                @if ($group->chairman_id)
                                                    ...
                                                    @if ($chairman)
                                                        <p>{{ $chairman->savers_surname }}
                                                            {{ $chairman->savers_given_name }}</p>
                                                    @endif
                                                @else
                                                    <code>
                                                        <a href="{{ route('admin.editGroup', ['id' => $group->id]) }}"
                                                            class="color-blue">Assign chairman</a>
                                        ...
                                        {{-- <div class="grid grid-cols-2">
                                                <div class="px-4 py-2 font-semibold">Total Members.</div>
                                                <div class="px-4 py-2">
                                                    <a class="text-blue-800" href="mailto:jane@example.com"> {{ $group->group_email }}</a>
                                                </div>
                                            </div> --}}
